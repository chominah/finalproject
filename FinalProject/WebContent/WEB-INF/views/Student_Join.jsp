...
<body>
	<div id="content">
		<form method="post" action="memberjoin" id="join">
			<fieldset>
				<legend>학생 회원가입 폼</legend>
				<table>
					<td style="width: 200px;">아이디 : &nbsp;</td>
					<td>
                     <input type="text" class="form-control" placeholder="아이디" name="sid" id="sid" required="required" style="width: 700px;">
					</td>
					<td>&nbsp; 
				...
				<table>
					...
					<input type="password" class="form-control"
						placeholder="비밀번호" name="spwd" id="spwd" required="required"
						pattern="[a-zA-Z]+[0-9]+"
				...
				<table>
					...
					<td><input type="text" id="spost" name="spost"
				...
				<table>
					...
					<td><input type="text" id="saddr1" name="saddr1"
				...
				<table>
					...
					<td><input type="text" id="saddr2" name="saddr2"
				...
				<table>
					...
					<td><input type="text" name="smail" id="smail"
				...
				<table>
					<div class="form-group">
						<td style="width: 200px;">신청 강좌 : &nbsp;
						</td>
						<td><select id="cname" name="cname" class="form-control" style="width: 500px;">
								<option>-선택하시오-</option>
								<c:forEach var="classvo" items="${classlist}">
									<option>${classvo.cname}</option>
								</c:forEach>
						</select></td>
					</div>
				</table>
		...
		$(function() {
					...
					function() {
						$.ajax({
							...
							success : function(data) {
								console.log(data);
								if (data == 1) {
									$('#chk_sid').show().css("color", "red")
											.text("사용불가능한 아이디입니다.");
								} else {
									$('#chk_sid').show().css("color", "blue")
											.text("사용가능한 아이디입니다.");
								}
							}
						});
			...
			var interVal = setInterval(function() {
				if ($('#spwd').val().length >= 6) {
					$('#chk_spwd').show().css("color", "blue").text(
							"적합한 길이입니다.");
				} else {
					$('#chk_spwd').show().css("color", "red").text(
							"6자리이상 입력해주세요.")
				}
			});
		});
