...
<title>admin_myInfo_modify.jsp</title>
</head>
<body>
<div id="content">
<form method="post" action="admin_myInfo_modify" id="modify">
<fieldset>
	<legend>관리자 회원수정 폼</legend>
				<table>
					<td style="width: 200px;">아이디 : &nbsp;</td>
					<td>
                     <input type="text" class="form-control" name="adid" id="adid"  value="${advo.adid}" required="required" style="width: 700px;">
					</td>
					<td>&nbsp; 
				...
				<table>
					...
					<input type="password" class="form-control"
						name="adpwd" id="adpwd" value="${advo.adpwd }" required="required"
						pattern="[a-zA-Z]+[0-9]+"
				...
				<table>
					...
					<td>
					<input type="text" class="form-control" value="${advo.adname }"
				...
				<table>
					...
					<td><input type="text" class="form-control" value="${advo.adbirth }"
				...
				<table>
					...
					<td><input type="text" id="adpost" name="adpost" value="${advo.adpost }"
				...
				<table>
					...
					<td><input type="text" id="adaddr1" name="adaddr1" value="${advo.adaddr1 }"
				...
				<table>
					...
					<td><input type="text" id="adaddr2" name="adaddr2" value="${advo.adaddr2 }"
				...
				<table>
					...
					<td><input type="text" name="adphone" id="adphone" value="${advo.adphone }"
				...
				<table>
					...
					<td><input type="text" name="admail" id="admail" value="${advo.admail }"
		...
		$('#idChkBtn').click(function(){
			$.ajax({
				...
				success:function(data){
					if(data==0){
						$('#chk_adid').show().css("color","blue").text("사용가능한 아이디입니다.");
					}else{
						$('#chk_adid').show().css("color","red").text("사용불가능한 아이디입니다.");
					}
				}
			});
		});
		...
		var interVal = setInterval(
			function(){
				if($('#adpwd').val().length >= 6) {
					$('#chk_adpwd').show().css("color","blue").text("적합한 길이입니다.");
				}else{
					$('#chk_adpwd').show().css("color","red").text("6자리이상 입력해주세요.")
				}
			}		
		);
	});
